...
4. **转变弧线**：每个节点的 change_before → change_after 是否形成强烈对立和有效的角色/情节弧线（转变前后应是同一维度的两个极端，而非渐进式变化）
...
3. 为每个节点填写：
   - 标题（贴合故事内容）
   - 转变前后状态（change_before / change_after）：必须形成强烈对立！
     ❌ 避免：转变前"面临挑战" → 转变后"克服困难"（太平淡）
     ✅ 应当：转变前"面临死亡" → 转变后"智取对手，成为英雄"（180度反转）
   - 每卷必须有核心转变，概括本卷最大的命运反转
   - 卷概要 / 核心冲突 / 地点和人物
4. 使用 batch_create_outline 工具一次性创建完整大纲

确保大纲结构完整、情节连贯、冲突递进、转变对立鲜明。用中文输出所有内容。`,
            },
          },
        ],
      };
